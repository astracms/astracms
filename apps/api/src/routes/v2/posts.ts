...
v2Posts.openapi(listPostsV2Route, async (c) => {
    try {
        ...
        const categories = queryParams.categories
            ? queryParams.categories.split(",").map((s) => s.trim()).filter(Boolean)
            : [];
        const excludeCategories = queryParams.excludeCategories
            ? queryParams.excludeCategories.split(",").map((s) => s.trim()).filter(Boolean)
            : [];
        const tags = queryParams.tags
            ? queryParams.tags.split(",").map((s) => s.trim()).filter(Boolean)
            : [];
        const excludeTags = queryParams.excludeTags
            ? queryParams.excludeTags.split(",").map((s) => s.trim()).filter(Boolean)
            : [];
        ...
        const where = {
            ...
            ...(() => {
                ...
                if (excludeCategories.length > 0) categoryFilter.notIn = excludeCategories;
            ...
            ...(() => {
                ...
                if (excludeTags.length > 0) tagFilter.none = { slug: { in: excludeTags } };
