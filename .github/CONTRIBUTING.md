# Contributing to Astracms

Thanks for your interest in contributing! This guide explains how to get Astracms running locally and how to submit high-quality contributions.
...
1. [Fork](https://github.com/astracms/astracms/fork/) this repository to your own account

   - Visit Astracms repository
   ...
   ```bash
   git clone https://github.com/YOUR-USERNAME/astracms.git
   cd astracms
   ```
   ...
   ```bash
   git remote add upstream https://github.com/astracms/astracms.git
   ```
...
3. Configure Environment Variables
   ...
   - Minio credentials for file uploads (see below)
...
4. Database Setup
   ...
   Example:
   ```bash
   DATABASE_URL=postgresql://astracms:justastracms@localhost:5432/astracms
   ```
...
- Persist data in the astracms_pgdata Docker volume.
...
### Set up Minio for media uploads

   To use media uploads in Astracms, you’ll need to set up a Minio server. Here's a step-by-step guide to help you configure everything properly:

- Start the Minio service using Docker Compose:
   ```bash
   pnpm docker:up
   ```
- Minio will be accessible at `http://localhost:9000` and the console at `http://localhost:9001`.
- The default credentials are `MINIO_ROOT_USER=minioadmin` and `MINIO_ROOT_PASSWORD=minioadmin`.
- Create a bucket named `astracms-media` in the Minio console.
- Set the following environment variables in `apps/cms/.env`:
   ```
   MINIO_ENDPOINT=http://localhost:9000
   MINIO_ACCESS_KEY=minioadmin
   MINIO_SECRET_KEY=minioadmin
   MINIO_BUCKET_NAME=astracms-media
   MINIO_PUBLIC_URL=http://localhost:9000/astracms-media
   ```

### Set up Upstash Redis

   Astracms uses Redis for rate limiting, session caching, and analytics. We use [Upstash](https://upstash.com) for serverless Redis. Here's how to set it up:

- Go to [Upstash Console](https://console.upstash.com) and sign in or create an account

- Click "Create Database"

- Give your database a name (e.g. astracms-redis)
